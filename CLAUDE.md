...
4. Отдельный PR `dev → staging` сюда (изменение схем) + отдельный PR `dev → staging`
   в каждый агент, где пришлось подправить логику под новые типы.
   После ревью — PR `staging → main` для деплоя.
